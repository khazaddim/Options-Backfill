# Massive Options Backfill Downloader Design

## Purpose

This document specifies a Python design for asynchronously downloading historical US options data from the Massive Options API into a dedicated PostgreSQL options database.

It builds on two existing pieces in this repository:

- `massive_options_helper.py`, which defines the Massive endpoint shapes, response normalization, pagination behavior, and DuckDB read-through cache concept.
- `tests/test_massive_options_helper.py`, which proves the adapter can flatten Massive-style responses and avoid repeat API calls through caching.
...
- Download Massive options data asynchronously for one or more underlying symbols.
...
- Keep provider-specific Massive logic isolated from storage and retry orchestration.
...
### `test_massive_options_helper.py`
...
Options-Backfill/
  docs/
    massive_options_backfill_downloader_design.md
  options_backfill/
    __init__.py
    config.py
    models.py
  ...
  tests/
    test_massive_options_helper.py
    test_massive_options_downloader.py
```

For a first version, this can start smaller with a single `massive_options_downloader.py` module. The split above is the direction to grow into once the workflow is proven.
...
```python
class MassiveOptionsBackfillDownloader:
...
- `requests_per_minute` should default well below the vendor maximum. Massive lists 1,000 requests/minute and 100,000 API calls/day, with one request costing 10 API calls. A conservative default such as 90 requests/minute leaves safety room.
...
- detailed Massive response parsing
...
The provider adapter should be the async version of the endpoint logic in `massive_options_helper.py`.
...
7. Worker calls MassiveOptionsProvider with rate limiter and retry policy.
...
- Milestones 0-1 are tracked by OpenSpec change `add-massive-backfill-m0-m1`.
...
- Must add:
    ...
    - storage implementation (`options_backfill/storage.py` or `massive_options_downloader.py`)
- May add:
...
Goal: fetch Massive pages asynchronously with normalized output.

Deliverables:

- `aiohttp` provider client.
- Endpoint URL/query builder reuse from `massive_options_helper.py`.
...
- Must add:
    - provider implementation (`options_backfill/massive_provider.py` or `massive_options_downloader.py`)
    - provider-focused tests (`tests/test_massive_options_downloader.py`)
- May add:
...
- `MassiveOptionsBackfillDownloader` class with worker pool.
...
- Must add:
    - downloader orchestration (`options_backfill/downloader.py` or `massive_options_downloader.py`)
...
The recommended design is a PostgreSQL-backed async downloader with Massive-specific provider logic isolated behind an adapter. The current `massive_options_helper.py` gives the endpoint and normalization foundation. The next layer should add durable job/task state, async pagination, rate limiting, retries, and idempotent PostgreSQL writes.
