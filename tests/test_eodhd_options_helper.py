...
import eodhd_options_helper as options_helper
...
def test_download_options_eod_merges_extended_tradetime_window_into_cache(
    monkeypatch,
    tmp_path: Path,
) -> None:
    monkeypatch.setattr(options_helper, "OPTIONS_CACHE_DB_PATH", tmp_path / "options_cache.duckdb")
    ...
    def fake_http_get_json(url: str, timeout: int = 30) -> dict[str, object]:
        ...
        if "2025-01-03" in url and "2025-01-10" not in url:
            return {
                "data": [
                    {
                        "id": "SPY250117P00450000-2025-01-03",
        ...
        return {
            "data": [
                ...
                {
                    "id": "SPY250117P00450000-2025-01-10",
                    "type": "options-eod",
                    "attributes": {
                        "contract": "SPY250117P00450000",
                        "underlying_symbol": "SPY",
...
def test_download_options_requires_api_token(monkeypatch, tmp_path: Path) -> None:
    ...
    try:
        ...
        assert "EODHD API token is required" in str(exc)
    else:
